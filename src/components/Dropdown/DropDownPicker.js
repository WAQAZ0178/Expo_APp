/* eslint-disable */
// All code is from source code in library 'react-native-dropdown-picker'
// imported here to use custom text component for font
...
class DropDownPicker extends React.Component {
  constructor(props) {
    ...
    if (!props.multiple) {
      ...
      } else if (props.items.filter(item => item.selected).length > 0) {
        choice = props.items.filter(item => item.selected)[0]
      } else {
        choice = this.null()
      }
    } else if (
      ...
      items = props.items.filter((item, index) => item.selected)
    }
  ...
  render() {
    ...
    const label = isPlaceholderActive
      ? placeholder
      : this.state.choice.label.substr(0, this.props.selectedLabelLength)
        ...
        >
          ...
          <ScrollView style={{ width: '100%' }} nestedScrollEnabled={true} {...scrollViewProps}>
            {items.length > 0 ? (
              ...
              <View style={styles.notFound}>{this.props.searchableError()}</View>
            )}
          </ScrollView>
        </View>
      </View>
...
DropDownPicker.defaultProps = {
  ...
  searchableError: () => <Text useGutters={false}>Not Found</Text>,
  searchableStyle: {},
